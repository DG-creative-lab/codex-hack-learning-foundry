...
export const sourceOutputsSchema = z.object({
...
const processingPayloadSchema = z.object({ sourceId: z.string().min(1), progress: z.number().min(0).max(100) }).strict();
...
export function deriveSources(events: EvidenceEvent[]): SourceRecord[] {
  ...
  for (const event of events) {
    if (event.type === "source.registered") {
      ...
      if (!event.sourceIds.includes(source.id)) throw new Error(`Source event ${event.id} is missing provenance for ${source.id}`);
    ...
    if (event.type === "source.processing_started") {
      ...
      if (!event.sourceIds.includes(sourceId)) throw new Error(`Source event ${event.id} is missing provenance for ${sourceId}`);
