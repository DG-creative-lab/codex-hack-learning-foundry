    ...
    <div className="page-scroll sources-view">
      <section className="pipeline-section">
        <div className="section-heading">
          <div><p className="eyebrow">Universal processing pipeline</p><h2>One source. Two learning paths.</h2></div>
      ...
      <section className="library-section">
        <div className="section-heading compact">
          <div><p className="eyebrow">Source library</p><h2>{sources.length} captured sources</h2></div>
          <div className="search-box"><Search size={14} /><input aria-label="Search sources" placeholder="Search sources" /></div>
        </div>
        <div className="source-workspace">
          ...
          <aside className="source-inspector">
            ...
            <dl>
              <div><dt>Provenance</dt><dd>{selectedSource.provenance}</dd></div>
              <div><dt>Added</dt><dd>{selectedSource.addedAt}</dd></div>
              <div><dt>Processing</dt><dd>{selectedSource.progress}%</dd></div>
            </dl>
            <div className="progress-track"><i style={{ width: `${selectedSource.progress}%` }} /></div>
            {selectedSource.status === "ready" ? (
              <div className="output-summary">
                <p><strong>{selectedSource.outputs.atoms}</strong><span>knowledge atoms</span></p>
                <p><strong>{selectedSource.outputs.lessons}</strong><span>learning modules</span></p>
                <p><strong>{selectedSource.outputs.capabilities}</strong><span>capabilities</span></p>
              </div>
            ) : (
              <button className="primary-button full" onClick={onProcess}>
                {selectedSource.status === "processing" ? <><RefreshCw className="spin" size={15} /> Processing</> : <><Sparkles size={15} /> Process source</>}
              </button>
            )}
          </aside>
        </div>
