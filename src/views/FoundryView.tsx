  ...
  return (
    <div className="page-scroll foundry-view">
      <section className="foundry-header">
        <div><p className="eyebrow">Agent capability state</p><h2>Knowledge becomes controlled action.</h2></div>
      ...
      <section className="capability-workspace">
        <div className="capability-list">
          <div className="capability-head"><span>Artifact</span><span>Version</span><span>Status</span></div>
          {capabilities.map((item) => (
            <button key={item.id} className={selectedId === item.id ? "selected" : ""} onClick={() => setSelectedId(item.id)}>
              <span><small>{item.type}</small><strong>{item.name}</strong></span><span>{item.version}</span><span>{item.status}</span>
            </button>
          ))}
        </div>
        <div className="capability-detail">
          <div className="detail-title"><div><p className="eyebrow">{capability.type}</p><h2>{capability.name}</h2></div><span className="status-stamp">{capability.status}</span></div>
          ...
          <div className="foundry-actions"><button className="secondary-button"><ExternalLink size={14} /> Inspect artifact</button><button className="primary-button" disabled={capability.status !== "evaluated"}><ShieldCheck size={14} /> Approve activation</button></div>
        </div>
      </section>
    </div>
  );
