...
function buildPrompt(request) {
  const context = request.promptBoundary.contextSections
  ...
  return [
    ...
    "",
    request.promptBoundary.instruction,
    "",
    context,
    "",
...
export function createCodexExecutionService(options = {}) {
  ...
  async function execute(rawRequest) {
    ...
    const result = await runProcess(
      command,
      ["-q", "-a", "suggest", "--no-project-doc", "--disable-response-storage", buildPrompt(request)],
    ...
    if (result.ok && result.stdout.trim().length >= 3) {
      return liveExecutionResponseSchema.parse({ ok: true, outputSummary: result.stdout, timing });
    }
