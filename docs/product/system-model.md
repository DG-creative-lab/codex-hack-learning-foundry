...
The current product includes the central Understanding workspace, distinct human, agent, and shared-theory projections, evidence-backed understanding-gap signals, deliberate capability activation gates, explicit practical-evidence consolidation, and prepared or consent-gated live Codex execution. Applying an active capability records its adapter, version, bounded context, input provenance, timing, result, and recoverable fallback attempts. Human feedback and micro-world observations can generate targeted review work, proposed theory revisions, and structured capability revision requests. The canonical projector recomputes proposals from their trigger evidence. Theory changes remain inert until human approval, while capability requests still require a materially revised artifact and fresh evaluation before they can enter the capability lifecycle.

The complete offline demo is an eight-stage projection over this same system: source, theory, explanation, experiment, transfer, capability approval, application, and revision. A typed prepared-demo definition owns its required artifact IDs, capability and adapter versions, destinations, and completion predicates. Source, theory, and explainer inspection are intentionally local navigation progress because merely opening a screen is not learning evidence. Later stages complete only when the exact prepared provenance chain exists; unrelated evidence and live execution cannot advance them. Restart permanently removes all runtime workspace activity and restores the source-controlled prepared seed through the ledger storage boundary; it does not introduce a second demo-state database.
