...
The complete offline demo is an eight-stage projection over this same system: source, theory, explanation, experiment, transfer, capability approval, application, and revision. Source, theory, and explainer inspection are intentionally local navigation progress because merely opening a screen is not learning evidence. Later stages complete only when their canonical events exist. Restart replaces runtime activity with the source-controlled prepared seed through the ledger storage boundary; it does not introduce a second demo-state database.
