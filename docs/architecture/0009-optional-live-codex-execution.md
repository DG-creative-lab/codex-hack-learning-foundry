...
- The renderer sends a strictly bounded capability snapshot to Electron. It cannot select a command, working directory, environment, credential, or arbitrary filesystem path.
...
- The prompt boundary records the instruction, included capability context, and explicitly excluded context. API credentials, private desktop activity, unapproved sources, and canonical ledger contents are excluded.
